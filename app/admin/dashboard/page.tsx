...
export default function AdminDashboard() {
  ...
  const handleCreateStaff = async (e: React.FormEvent) => {
    ...
    if (staffList.length >= config.max_staff) {
      alert(`Limite atteinte pour le pack ${agencyProfile.pack}.`);
      return;
    }
    setUpdating(true);
  ...
  // Sécurité anti-crash : Ne rien rendre tant que le client n'est pas prêt
          ...
          <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in duration-500">
            <div className="bg-white/[0.02] p-8 rounded-[2rem] border border-white/5 backdrop-blur-3xl flex flex-col md:flex-row justify-between items-start md:items-end gap-6 shadow-2xl">
              ...
              <div className="flex gap-3">
                 <button className="p-4 bg-white/5 rounded-2xl hover:bg-white/10 transition-all border border-white/5"><Globe size={20}/></button>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="bg-[#0F172A]/80 backdrop-blur-2xl p-8 rounded-[2.5rem] border border-white/10 flex flex-col h-[450px] shadow-2xl">
                ...
                <div className="flex-1 space-y-3 overflow-y-auto pr-2 custom-scrollbar">
                  {documents.length === 0 ? (
                    <p className="text-[10px] text-slate-500 italic mt-10 text-center">Aucun document chargé.</p>
                  ) : (
                    documents.map((doc) => (
        ...
        <div className="fixed inset-0 bg-black/95 backdrop-blur-xl z-[60] flex items-center justify-center p-4">
          <form onSubmit={handleCreateStaff} className="bg-[#0F172A] w-full max-w-md rounded-[3rem] p-10 border border-white/10 shadow-2xl space-y-8 text-left">
            ...
            <button type="submit" disabled={updating} className="w-full bg-emerald-500 text-black py-6 rounded-2xl font-black uppercase text-[11px] hover:bg-white transition-all shadow-xl shadow-emerald-500/10">
              {updating ? <Loader2 className="animate-spin mx-auto"/> : 'Enregistrer & Créer PIN'}
            </button>
          </form>
        </div>
      )}
          ...
          <form onSubmit={handleCreateDossier} className="bg-[#0F172A] w-full max-w-4xl rounded-[3rem] p-10 border border-white/10 text-left space-y-8 my-auto shadow-2xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
               ...
               <div className="space-y-4">
                  ...
                  <input required placeholder="Nom de la Villa" className="w-full p-4 bg-emerald-500/10 text-emerald-400 rounded-xl border border-emerald-500/20 font-black uppercase outline-none" onChange={e => setNewDossier({...newDossier, nom_villa: e.target.value})} />
                  <div className="grid grid-cols-2 gap-4">
