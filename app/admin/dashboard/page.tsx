...
const TRANSLATIONS = {
  fr: {
    ...
    settings: "Profil & Agence", save: "Enregistrer", logout: "Déconnexion",
    ...
    <div className="min-h-screen bg-[#020617] flex flex-col md:flex-row text-slate-200 font-sans">
      ...
      <aside className="w-full md:w-80 bg-[#0F172A]/50 backdrop-blur-xl border-r border-white/5 h-screen sticky top-0 flex flex-col z-20">
        ...
        <div className="p-6 border-b border-white/5 flex flex-col gap-4">
          ...
          <div className="flex items-center gap-4 mt-2">
            ...
            <div>
              ...
              <p className="text-[10px] text-emerald-400 font-bold uppercase tracking-tighter italic">Espace Administration</p>
            </div>
          </div>
        </div>
            ...
            {activeTab === 'clients' ? (
              ...
              staffList.map((s) => (
                <div key={s.id} className="flex items-center justify-between p-4 rounded-xl border border-white/5 bg-white/5 group">
                  ...
                  <button onClick={async () => { if(confirm("Supprimer l'expert ?")) { await supabase.from('staff_prestataires').delete().eq('id', s.id); loadData(); }}} className="text-slate-600 group-hover:text-red-500 transition-colors"><Trash2 size={14}/></button>
                </div>
              ))
            )}
          </div>
        ...
        <div className="p-6 border-t border-white/5 bg-black/20">
          <button onClick={() => { supabase.auth.signOut(); window.location.href = '/'; }} className="w-full flex items-center justify-between p-4 rounded-xl text-slate-400 hover:bg-red-500/10 hover:text-red-500 transition-all group">
      ...
      <main className="flex-1 p-6 md:p-12 overflow-y-auto bg-gradient-to-br from-[#020617] to-[#0F172A]">
        {selectedProjet ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="bg-black/60 p-10 rounded-[2.5rem] border border-white/5 h-full">
                   ...
                   <div className="relative">
                      ...
                      <p className="text-2xl text-slate-200 italic font-medium leading-relaxed pl-4">
                        "{selectedProjet.commentaires_etape || "Aucun commentaire disponible pour cette phase du chantier."}"
                      </p>
                   </div>
                </div>
              </div>
              ...
              <div className="bg-[#0F172A]/80 backdrop-blur-2xl p-8 rounded-[2.5rem] border border-white/10 flex flex-col h-[500px] shadow-2xl">
                <div className="flex justify-between items-center mb-8">
                  ...
                  <label className="cursor-pointer p-3 bg-emerald-500 text-black rounded-xl hover:scale-110 active:scale-95 transition-all shadow-lg shadow-emerald-500/20">
                ...
                <div className="flex-1 space-y-3 overflow-y-auto pr-2 custom-scrollbar">
                  {documents.length === 0 ? (
                    <div className="h-full flex flex-col items-center justify-center text-center space-y-4 opacity-30">
                      ...
                      <p className="text-[10px] uppercase font-bold tracking-widest">{t.noDocs}</p>
                    </div>
                  ) : documents.map((doc) => (
                    <div key={doc.id} className="flex items-center justify-between p-4 bg-white/[0.03] rounded-2xl border border-white/5 hover:bg-white/5 transition-all group">
                      <div className="flex-1 mr-4 overflow-hidden">
                        <p className="text-[11px] font-bold truncate text-slate-300 group-hover:text-white transition-colors">{doc.nom_fichier}</p>
                        <p className="text-[8px] uppercase text-slate-600 font-black">Document Client</p>
                      </div>
                      <div className="flex gap-1">
                        <a href={doc.url_fichier} target="_blank" className="p-2 text-slate-500 hover:text-white transition-colors"><Download size={14} /></a>
                        <button onClick={async () => { if(confirm("Supprimer ?")) { await supabase.from('documents_projets').delete().eq('id', doc.id); loadDocuments(selectedProjet.id); }}} className="p-2 text-slate-500 hover:text-red-400 transition-colors"><Trash2 size={14} /></button>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          /* État vide / Accueil Dashboard */
          <div className="h-full flex flex-col items-center justify-center">
            <div className="relative">
              <div className="absolute inset-0 blur-3xl bg-emerald-500/10 rounded-full"></div>
              <Zap size={100} className="text-emerald-500 mb-8 relative animate-pulse" />
            </div>
      ...
      {/* --- MODALES (Inchangées mais incluses pour le code complet) --- */}
      {/* MODALE DOSSIER */}
      {showModal && (
            ...
            <form onSubmit={handleCreateDossier} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  ...
                  <input required placeholder="Prénom" className="w-full bg-black/40 p-4 rounded-xl border border-white/5 outline-none focus:border-emerald-500" value={newDossier.client_prenom} onChange={e => setNewDossier({...newDossier, client_prenom: e.target.value})} />
                  <input required placeholder="Nom" className="w-full bg-black/40 p-4 rounded-xl border border-white/5 outline-none focus:border-emerald-500" value={newDossier.client_nom} onChange={e => setNewDossier({...newDossier, client_nom: e.target.value})} />
                  <input required type="email" placeholder="Email" className="w-full bg-black/40 p-4 rounded-xl border border-white/5 outline-none focus:border-emerald-500" value={newDossier.email_client} onChange={e => setNewDossier({...newDossier, email_client: e.target.value})} />
                </div>
                <div className="space-y-4">
                  <label className="text-[9px] font-black text-blue-400 uppercase ml-2 tracking-widest">Localisation</label>
                  <input placeholder="Adresse" className="w-full bg-black/40 p-4 rounded-xl border border-white/5 outline-none focus:border-blue-500" value={newDossier.rue} onChange={e => setNewDossier({...newDossier, rue: e.target.value})} />
                  <input placeholder="Ville" className="w-full bg-black/40 p-4 rounded-xl border border-white/5 outline-none focus:border-blue-500" value={newDossier.ville} onChange={e => setNewDossier({...newDossier, ville: e.target.value})} />
                  <input placeholder="Pays" className="w-full bg-black/40 p-4 rounded-xl border border-white/5 outline-none focus:border-blue-500" value={newDossier.pays} onChange={e => setNewDossier({...newDossier, pays: e.target.value})} />
                </div>
              </div>
              <div className="pt-4 border-t border-white/5 grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                    ...
                    <input required placeholder="Nom de la Villa" className="w-full bg-black/40 p-4 rounded-xl border border-white/5 outline-none focus:border-purple-500" value={newDossier.nom_villa} onChange={e => setNewDossier({...newDossier, nom_villa: e.target.value})} />
                ...
                <div className="space-y-4">
                    ...
                    <input type="number" placeholder="Cashback (€)" className="w-full bg-black/40 p-4 rounded-xl border border-white/5 outline-none focus:border-amber-500" value={newDossier.montant_cashback} onChange={e => setNewDossier({...newDossier, montant_cashback: Number(e.target.value)})} />
                </div>
              </div>
        ...
        <div className="fixed inset-0 bg-black/95 backdrop-blur-xl z-[100] flex items-center justify-center p-6">
          <div className="bg-[#0F172A] w-full max-w-md rounded-[2.5rem] p-10 border border-white/10 shadow-2xl relative">
            ...
            <form onSubmit={handleCreateStaff} className="space-y-4">
              ...
              <input type="text" placeholder="PIN Personnel (Optionnel)" className="w-full p-5 bg-black/40 rounded-2xl border border-white/5 outline-none focus:border-emerald-500 font-mono text-center tracking-widest text-xl" maxLength={6} value={newStaff.pin} onChange={e => setNewStaff({...newStaff, pin: e.target.value})} />
              <button type="submit" disabled={updating} className="w-full bg-emerald-500 text-black py-6 rounded-[2rem] font-black uppercase text-xs tracking-widest mt-6">
