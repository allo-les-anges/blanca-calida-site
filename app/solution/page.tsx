...
      steps: [
        { icon: MapPin, t: "1. Local Need", d: "Your client wants to invest abroad. You gain access to a certified international database." },
        { icon: Layers, t: "2. Activation", d: "Instantly access a unified and verified portfolio of global properties." },
        { icon: Camera, t: "3. Real-Time Tracking", d: "On-site pros upload progress reports and photos directly to your platform." },
        { icon: ShieldCheck, t: "4. Total Trust", d: "Clients access their dedicated dashboard to follow investments in real-time." }
      ]
    },
    saas: { 
...
  return (
    <div className="bg-[#020617] text-white min-h-screen font-sans selection:bg-emerald-500/30 transition-all duration-500 relative" dir={t.dir}>
      
      {/* --- 1. FONDS GLOBAL (Photo 1.jpg) --- */}
...
            </button>
            {isLangOpen && (
              <div className="absolute right-0 mt-3 w-44 bg-[#0f172a] border border-white/10 rounded-xl overflow-hidden shadow-2xl z-50 overflow-y-auto max-h-64">
                {(Object.keys(translations) as Array<keyof typeof translations>).map((l) => (
                  <button
                    key={l}
                    onClick={() => { setLang(l); setIsLangOpen(false); }}
                    className={`w-full text-left px-5 py-3 text-[10px] font-bold uppercase tracking-widest hover:bg-[#10B981] hover:text-white transition-colors flex justify-between items-center ${lang === l ? 'text-[#10B981] bg-white/5' : 'text-slate-400'}`}
                  >
                    {translations[l].label}
                    {lang === l && <div className="w-1.5 h-1.5 bg-[#10B981] rounded-full"></div>}
                  </button>
                ))}
              </div>
            )}
...
      {/* --- HERO --- */}
      <section id="vision" className="relative pt-24 pb-16 px-6 text-center z-10">
...
      {/* --- SECTION FRACTURE (Photo 2.jpg en ratio 16:9) --- */}
      <section className="py-24 px-6 bg-emerald-950/10 border-y border-white/5 z-10 relative">
...
            </h2>
            <div className="space-y-6 text-slate-400 font-light leading-relaxed text-lg text-justify relative z-10">
              <p>{t.gap.desc1}</p>
...
          <div className="relative aspect-[16/9] rounded-3xl overflow-hidden border border-emerald-500/30 bg-slate-900 group shadow-2xl shadow-emerald-950/20">
             <Image 
               src="/2.jpg"
               alt="Real Estate Gap Visualization"
               fill
               className="object-cover group-hover:scale-105 transition-transform duration-700 opacity-90 group-hover:opacity-100"
             />
             <div className="absolute inset-0 bg-gradient-to-t from-[#020617] via-transparent to-transparent opacity-60"></div>
             <Cpu className="absolute bottom-4 right-4 w-10 h-10 text-[#10B981]/40 animate-pulse z-10" />
          </div>
        </div>
      </section>

      {/* --- HOW IT WORKS (Photos 3.1.jpg à 3.4.jpg en ratio 16:9) --- */}
...
              // Génération du nom de l'image correspondante (3.1.jpg, etc.)
              const imgNum = i + 1;
              return (
                <div key={i} className="bg-white/5 rounded-3xl border border-white/5 hover:border-emerald-500/30 transition-all text-left group overflow-hidden flex flex-col shadow-xl hover:shadow-emerald-950/30 hover:-translate-y-1 duration-300">
                  {/* Conteneur Image 16:9 */}
                  <div className="relative aspect-[16/9] w-full overflow-hidden border-b border-white/5">
                    <Image 
                      src={`/3.${imgNum}.jpg`}
                      alt={`Step ${imgNum} visual`}
                      fill
                      className="object-cover group-hover:scale-110 transition-transform duration-500 opacity-80 group-hover:opacity-100"
                    />
                    <div className="absolute inset-0 bg-gradient-to-t from-slate-950/80 to-transparent"></div>
                    <div className="absolute bottom-3 left-4 w-10 h-10 bg-[#020617]/80 rounded-full flex items-center justify-center text-[#10B981] border border-emerald-500/20 backdrop-blur-sm">
                      <Icon size={18} />
                    </div>
                  </div>
                  <div className="p-6 flex-grow flex flex-col justify-start">
                    <h3 className="font-black text-[10px] uppercase tracking-widest text-white mb-2 group-hover:text-[#10B981] transition-colors">{step.t}</h3>
...
      {/* --- SAAS MASTER TEMPLATE SECTION (Photo 4.jpg en ratio 16:9) --- */}
      <section id="saas" className="py-24 px-6 bg-slate-950 border-y border-white/5 relative overflow-hidden z-10">
...
              const Icon = f.icon;
              return (
                <div key={i} className="p-8 bg-white/5 border border-white/5 rounded-3xl hover:border-emerald-500/20 transition-all group">
                  <Icon className="text-[#10B981] mb-4 transition-transform group-hover:scale-110" size={24} />
...
            {/* Conteneur Image 16:9 de la console (Photo 4.jpg) */}
            <div className="relative aspect-[16/9] w-full rounded-2xl overflow-hidden border border-emerald-500/20 bg-slate-900 shadow-2xl shadow-emerald-950/30 group">
              <Image 
                src="/4.jpg"
                alt="SaaS Administration Console"
                fill
...
            <button className="flex items-center gap-2 text-[#10B981] font-black uppercase text-[10px] tracking-widest group pt-4">
...
           <div className="relative h-8 w-24 grayscale hover:grayscale-0 transition-all duration-500">
              <Image src="/logo.jpeg" alt="Logo Footer" fill className="object-contain" />
           </div>
        </div>
        <p className="text-[8px] uppercase tracking-[0.2em]">© 2026 Data-Home.io / Global Infrastructure Engine / Mons, Belgium</p>
      </footer>
    </div>
  );
}