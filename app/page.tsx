  ...
  return (
    <main className="bg-[#020617] min-h-screen text-slate-200 selection:bg-[#D4AF37]/30 font-sans overflow-x-hidden">
      ...
      <div className="relative h-screen flex flex-col items-center justify-center">
        ...
        {/* BOUTON POSITIONNÉ SOUS LE TEXTE DU HERO */}
        <div className="absolute bottom-[20%] z-40">
           {!isSearchOpen && (
             <button 
                onClick={() => setIsSearchOpen(true)}
                className="group flex items-center gap-6 bg-black/40 backdrop-blur-xl border border-white/10 px-8 py-4 rounded-full hover:border-[#D4AF37]/50 transition-all duration-500 shadow-2xl scale-90 md:scale-100"
             >
                <div className="w-10 h-10 bg-[#D4AF37] rounded-full flex items-center justify-center text-black group-hover:rotate-90 transition-transform duration-500">
                  <Search size={18} />
                </div>
                <div className="text-left">
                  <span className="block text-[9px] font-bold text-[#D4AF37] uppercase tracking-[0.3em]">Recherche Privée</span>
                  <span className="block text-white font-serif italic text-base">Trouver votre villa</span>
                </div>
             </button>
           )}
        </div>
      ...
      {/* MODAL DE RECHERCHE (BARRE BLANCHE DU SCREENSHOT) */}
      <div className={`max-w-7xl mx-auto px-6 relative z-50 transition-all duration-700 ease-in-out ${isSearchOpen ? 'opacity-100 -mt-64' : 'opacity-0 pointer-events-none mt-0'}`}>
        <div className="bg-white p-2 rounded-[2.5rem] shadow-[0_50px_100px_-20px_rgba(0,0,0,0.5)] relative">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center relative z-10">
            <div className="space-y-10">
              <div className="space-y-4">
                ...
                <h2 className="text-5xl md:text-7xl font-serif text-white leading-[1.1] italic">Suivez votre vision <br /> <span className="text-[#D4AF37] not-italic font-sans font-light tracking-tighter text-4xl md:text-6xl">en temps réel.</span></h2>
              </div>
              <p className="text-slate-400 text-lg font-light leading-relaxed max-w-lg border-l border-white/10 pl-8 italic">Votre projet, accessible partout, à chaque étape du chantier.</p>
            </div>
            <div className="bg-[#020617]/80 p-12 rounded-[2rem] border border-[#D4AF37]/20 backdrop-blur-xl">
               <form onSubmit={handleClientLogin} className="space-y-8 text-center">
                  ...
                  <div className="relative group">
                    <input 
                      ...
                      className="w-full bg-transparent border-b border-white/10 py-6 text-center text-3xl font-light tracking-[1em] text-white outline-none focus:border-[#D4AF37] transition-all"
                    />
                  </div>
                  <button className="w-full bg-[#D4AF37] text-black py-6 rounded-full font-bold text-[11px] uppercase tracking-[0.3em] hover:bg-white transition-all flex items-center justify-center gap-4">
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          {!hasActiveFilters && visibleCount < filteredProperties.length && (
            <div className="text-center mt-32">
              <button onClick={() => setVisibleCount((prev) => prev + 12)} className="px-20 py-8 border border-[#D4AF37]/40 text-[#D4AF37] rounded-full uppercase text-[10px] tracking-[0.5em] font-bold hover:bg-[#D4AF37] hover:text-black transition-all">
                Voir plus
              </button>
            </div>
          )}
