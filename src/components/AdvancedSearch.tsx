  ...
  const towns = useMemo(() => {
    ...
    return Array.from(
      new Set(
        properties
          .map((p: any) => p.town)
          .filter((town: string) => town && town.trim() !== "")
      )
    ).sort();
  }, [properties]);

  // ===============================
  // LISTE DES TYPES (CORRIGÉE)
  // ===============================
  const types = useMemo(() => {
    if (!properties || properties.length === 0) return [];
    ...
    properties.forEach((p: any) => {
      // ⚠️ adapte ici si ton XML utilise un autre nom
      const type =
        p.property_type ||
        p.type ||
        p.category ||
      ...
      <div className="bg-white shadow-2xl p-8 border border-gray-100">
        <div className="grid grid-cols-1 md:grid-cols-6 gap-6 mb-8">
          ...
          <div className="flex flex-col border-b border-gray-200 pb-2">
            ...
            >
              <option value="">
                Toutes les villes ({towns.length})
              </option>
              {towns.map((town) => (
