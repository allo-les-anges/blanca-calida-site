...
export default function PropertyDetailClient({ id }: { id: string }) {
  ...
  const cleanDescription = (html: string) => {
    if (!html) return "";
    return html.replace(/style="[^"]*"/gi, '').replace(/color="[^"]*"/gi, '').replace(/<font[^>]*>/gi, '').replace(/<\/font>/gi, '').replace(/ /g, ' ');
  };

  if (!mounted) return null;

  if (loading) return (
    <div className={`h-screen flex flex-col items-center justify-center ${isLight ? 'bg-white' : 'bg-white dark:bg-[#0A0A0A]'} text-[#D4AF37]`}>
  ...
  if (!property) return (
    <div className={`h-screen flex flex-col items-center justify-center ${isLight ? 'bg-white' : 'bg-white dark:bg-[#0A0A0A]'} p-6 text-center`}>
      <h1 className={`text-2xl mb-4 font-serif ${isLight ? 'text-slate-900' : 'dark:text-white'}`}>{t('propertyDetail.propertyNotFound')}</h1>
  ...
  return (
    <main className={`min-h-screen transition-colors duration-500 ${isLight ? 'bg-white' : 'bg-white dark:bg-[#0A0A0A]'}`}>
      <Navbar />
      ...
      <section className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-3 gap-16 pb-24">
        <div className="lg:col-span-2">
          <h1 className={`text-4xl md:text-7xl font-serif mb-8 leading-[1.1] ${isLight ? 'text-slate-900' : 'text-slate-900 dark:text-white'}`}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-16">
            {[
              ...
              <div key={i} className={`${isLight ? 'bg-slate-100' : 'bg-slate-50 dark:bg-white/5'} p-6 rounded-3xl text-center border ${isLight ? 'border-slate-200' : 'border-white/10'}`}>
                <item.icon className="mx-auto mb-2 text-[#D4AF37]" size={22} />
                <p className={`text-2xl font-serif ${isLight ? 'text-slate-900' : 'text-slate-900 dark:text-white'}`}>{item.val || "0"}</p>
                <p className="text-[8px] uppercase text-slate-400 font-bold tracking-widest">{t(`propertyDetail.${item.key}`)}</p>
              </div>
            ))}
          </div>

          <div className={`max-w-none mb-20 pt-10 border-t ${isLight ? 'border-slate-200' : 'border-white/10'}`}>
            <h2 className={`text-3xl font-serif italic mb-8 ${isLight ? 'text-slate-900' : 'dark:text-white'}`}>{t('propertyDetail.artOfLiving')}</h2>
            <div 
              className={`text-lg leading-relaxed opacity-90 mb-16 ${isLight ? 'text-slate-700' : 'dark:text-slate-300'}`}
            ...
            <div className="space-y-8">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className={`text-2xl font-serif italic ${isLight ? 'text-slate-900' : 'dark:text-white'}`}>{t('propertyDetail.location')}</h3>
              ...
              <div className="relative w-full h-[400px] rounded-[2.5rem] overflow-hidden shadow-xl border border-slate-200 dark:border-white/10">
                <iframe width="100%" height="100%" frameBorder="0" scrolling="no" src={fallbackMapUrl} className={isLight ? "" : "grayscale-[0.5] dark:grayscale-[1]"} />
              </div>
            </div>
          </div>
        </div>
            ...
            <div className={isLight ? "" : "bg-white dark:bg-[#111] border border-slate-200 dark:border-white/10 rounded-[3rem] shadow-2xl overflow-hidden"}>
              {!isLight && (
                <div className="p-10 pb-0">
                  ...
                  <p className={`text-5xl font-serif leading-none mb-6 ${isLight ? 'text-slate-900' : 'dark:text-white'}`}>
                    {numericPrice.toLocaleString("fr-FR")} €
                  </p>
                </div>
              )}
              <ContactForm agency={currentAgency} propertyRef={property.id_externe || property.id} />
              <div className="px-8 pb-8">
                <a href={`https://wa.me/34627768233?text=Info ref: ${property.ref}`} target="_blank" className={`w-full border ${isLight ? 'border-slate-200' : 'border-white/10'} flex items-center justify-center gap-3 py-4 rounded-2xl font-bold uppercase text-[10px] hover:bg-slate-50 transition-all ${isLight ? 'text-slate-900' : 'dark:text-white'}`}>
