...
// ETAPE 1 : Définition précise du dictionnaire de correspondance
const CITY_TO_REGION_MAP: Record<string, string> = {
  // COSTA BLANCA (Province d'Alicante)
  ...
  // COSTA DEL SOL (Province de Malaga / Cadix)
  ...
  // COSTA CALIDA (Province de Murcie)
  "murcia": "Costa Calida",
  ...
  // COSTA ALMERIA (Province d'Almeria)
...
const REGIONS_DISPLAY = [
  { name: "Costa Blanca", image: "https://images.unsplash.com/photo-1589394815804-964ed7be2eb5?auto=format&fit=crop&q=80" },
  { name: "Costa del Sol", image: "https://images.unsplash.com/photo-1512753360425-4223d5ba5952?auto=format&fit=crop&q=80" },
  { name: "Costa Calida", image: "https://images.unsplash.com/photo-1590001158193-7903d2e5eec1?auto=format&fit=crop&q=80" },
  { name: "Costa Almeria", image: "https://images.unsplash.com/photo-1564507004663-b6dfb3c824d5?auto=format&fit=crop&q=80" }
];

export default function RegionGrid({ properties, onRegionClick }: RegionGridProps) {
  
  // ETAPE 2 : Comparaison avec la base de données
  const regionCounts = useMemo(() => {
    ...
    properties.forEach(p => {
      // 1. On récupère la ville du CSV (soit 'town' soit 'ville')
      const rawCity = (p.town || p.ville || "").toLowerCase().trim();
      
      // 2. On regarde si cette ville est dans notre dictionnaire
      const regionFound = CITY_TO_REGION_MAP[rawCity];
      
      // 3. Si oui, on incrémente le compteur de la région correspondante
      if (regionFound) {
        counts[regionFound]++;
      } else {
        // Optionnel : Si la colonne 'region' du CSV est déjà correcte, on peut l'utiliser en secours
        if (p.region && counts[p.region] !== undefined) {
          counts[p.region]++;
        }
      }
    });
        ...
        return (
          ...
          >
            <div className="absolute inset-0">
              <img src={region.image} alt={region.name} className="w-full h-full object-cover transition-transform duration-1000 scale-105 group-hover:scale-110 opacity-60 group-hover:opacity-40" />
            ...
            <div className="absolute inset-0 z-10 p-10 flex flex-col items-center text-center">
              ...
              <div className="space-y-6">
                <h3 className="text-4xl md:text-5xl font-serif italic text-white leading-tight">{region.name}</h3>
